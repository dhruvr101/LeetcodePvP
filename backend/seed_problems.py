...
# ---------------------------
# Solvers (correct reference implementations)
# ---------------------------
...
def generate_merge_sorted_lists_cases(n_cases=100):
    cases = []
    for _ in range(n_cases):
        list1 = sorted(random.sample(range(1, 50), random.randint(1, 5)))
        list2 = sorted(random.sample(range(1, 50), random.randint(1, 5)))
...
def generate_median_sorted_arrays_cases(n_cases=100):
    ...
    for _ in range(n_cases):
        ...
        nums1 = sorted(random.sample(range(1, 50), size1))
        nums2 = sorted(random.sample(range(50, 100), size2))
...
for problem in problems.find():
    ...
    if title in GENERATORS:
        ...
        print(f"✅ Updated {title} with {len(cases)} advanced test cases")
    else:
        print(f"⚠️ No generator for {title}, skipped")
